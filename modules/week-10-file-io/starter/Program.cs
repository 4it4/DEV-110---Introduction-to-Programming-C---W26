...
public class Program
{
    ...
    private static void PrintSummary(List<Habit> habits)
    {
        ...
        int totalDaily = habits.Count(h => h.Frequency == "daily");
        ...
        if (totalDaily > 0)
        {
            double dailyRate = (double)completedDaily / totalDaily * 100;
            Console.WriteLine($"Daily:    {completedDaily}/{totalDaily} completed ({dailyRate.ToString("F1", CultureInfo.InvariantCulture)}%)");
        }
        else
        {
            Console.WriteLine("Daily:    0/0 completed (0.0%)");
        }

        if (totalWeekly > 0)
        {
            double weeklyRate = (double)completedWeekly / totalWeekly * 100;
            Console.WriteLine($"Weekly:   {completedWeekly}/{totalWeekly} completed ({weeklyRate.ToString("F1", CultureInfo.InvariantCulture)}%)");
        }
        else
        {
            Console.WriteLine("Weekly:   0/0 completed (0.0%)");
        }
    }
    ...
    private static void UpdateHabit(List<Habit> habits)
    {
        ...
        string status = habit.IsCompleted ? "completed" : "pending";
        Console.WriteLine($"Updated: {habit.Name} \u2014 now {status}");
    }
